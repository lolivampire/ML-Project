...
(list 5 topik + 1 baris deskripsi singkat tiap topik)
...
## Output Files
(list semua script yang dihasilkan + keterangan singkat fungsinya)
...
## Cara Menjalankan
(instruksi singkat: aktivasi venv, install requirements, cara run tiap script)
...
## Key Learnings
(3–5 poin paling penting yang kamu pelajari minggu ini — tulis dengan kata-katamu sendiri)
...
4. Matplotlib adalah dasar, dibuat dari nol tapi user memiliki akses penuh modifikasi dan pembuatan, sedangkan seaborn adalah template dsign jadi, lebih cantik secara visual namun terbatas secara kontrol.
...
6. Struktur file sangatlah penting agar kode menjadi clean, reusable, dan mudah dipahami gunanakan struktur constants → functions → main `if __name__ == "__main__":` pada bagian main untuk menjalankan kode